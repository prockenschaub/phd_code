...
# Patients enter the cohort at any time (includes both < and > 65)
patients <- eligible
patients[, c("start_date", "end_date") := NULL]

def_patients <- copy_to(get_db_conn(), patients, name = "patients")

attrition$`2_lt_65`<- nrow(patients[turn_65 > enter_date])
attrition$`2_lt_65_fu` <- 
  patients[turn_65 > enter_date, 
           sum(time_length(enter_date %--% turn_65, u = "y"))]

attrition$`2_gt_65` <- nrow(patients[turn_65 < leave_date])
attrition$`2_gt_65_fu` <- 
  patients[turn_65 < leave_date, 
           sum(time_length(pmax(turn_65, enter_date) %--% leave_date, u = "y"))]
...
mget(char(patients, region, uti_prim, utest_prim, uti_sec, 
