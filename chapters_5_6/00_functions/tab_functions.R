...
tab_OR <- function(dt, outcome, var, group, CI = TRUE, alpha = 0.05){
  # Calculate the crude odds ratio between an outcome (`var`) and an exposure
  # (`group`).
  #
  # Args:
  #   dt : data.table 
  #     data containing columns `var` and `group`
  #   outcome : character 
  #     name of the exposure variable (factor or numeric)
  #   var : character
  #     name of the binary outcome variable
  #   group : IGNORED
  #     only included for compatibility
  #   CI : logical
  #     flag defining whether an approximate confidence interval is calculated
  #   alpha : numeric, default 0.05
  #     alpha level (i.e. probability not captured in CI)
  #
  # Result:
  #   a data.table with all levels of `group` in a var column and 
  
  mod <- glm(as.formula(str_c(outcome, "~", var)), 
             family = binomial(link = "logit"), data = dt)
  
  betas <- tidy(mod)
  setDT(betas)
  
  betas[, OR := prty(exp(estimate), 2)]
  
  if(CI){
    betas[, lower := exp(estimate + qnorm(alpha / 2) * std.error)]
    betas[, upper := exp(estimate + qnorm(1 - alpha / 2) * std.error)]
    betas[, OR := str_c(OR, " [", prty(lower, 2), ", ", prty(upper, 2), "]")]
  }
  
  if(is.factor(dt[[var]])){
    betas[, term := levels(dt[[var]])]
    betas[1, OR := "1"]
  } else {
    betas[, term := "linear"]
    betas %<>% .[-1, ]
  ...
  betas[, .(var = term, .all = ".all", value = OR)]
}


tab_test <- function(dt, var, group){
  
  if(is.factor(dt[[var]])){
    cont <- table(dt[[var]], dt[[group]])
    pval <- chisq.test(cont)$p.val
    res <- data.table(var = levels(dt[[var]])[1], value = pval)
    
  } else {
    pval <- kruskal.test(dt[[var]], dt[[group]])$p.value
    res <- data.table(var = "p-value", value = pval)
  }
  
  res[, value := ifelse(value < 0.001, "<0.001", prty(value, 3))]
  res[, (group) := group]
  res[]
}
