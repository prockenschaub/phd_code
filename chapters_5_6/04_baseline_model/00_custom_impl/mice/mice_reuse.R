...
mice.reuse <- function(mids, newdata, maxit = 5, printFlag = TRUE, seed = NA){
  ...
  # Set up a mids object for the newdata, but artificially flag all values 
  # of variables that need to imputed to missing
  all_miss <- matrix(TRUE, rows, cols, dimnames = list(seq_len(rows), nm))
  all_miss[, names(mids$method[mids$method == ""])] <- FALSE
